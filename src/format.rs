...
impl StringFormat {
    ...
    pub fn write<T: io::Write>(&self, out: &mut T, ts: &Timestamp) -> io::Result<()> {
        ...
        match self {
            ...
            StringFormat::UtcTime | StringFormat::LocalTime => write!(
                out,
                "{hour:02}:{mins:02}:{secs:02}",
                hour = parts.hour,
                mins = parts.minutes,
                secs = parts.seconds
            ),
            ...
            | StringFormat::LocalHTTP
            | StringFormat::LocalRFC7231 => write!(
                out,
...
mod test_format {
    ...
    fn timestamp_as_utc_string() {
        ...
        assert_eq!(
            StringFormat::UtcDateTime.as_string(&ts),
            "2026-03-06 14:43:49"
        );
        assert_eq!(
            StringFormat::UtcMillisDateTime.as_string(&ts),
            "2026-03-06 14:43:49.038"
        );
        assert_eq!(
            StringFormat::UtcNanosDateTime.as_string(&ts),
            "2026-03-06 14:43:49.038023456"
        );
        assert_eq!(
            StringFormat::UtcFileName.as_string(&ts),
            "2026-03-06_14-43-49"
        );
        assert_eq!(StringFormat::UtcTime.as_string(&ts), "14:43:49");
        assert_eq!(StringFormat::UtcMillisTime.as_string(&ts), "14:43:49.038");
        assert_eq!(
            StringFormat::UtcNanosTime.as_string(&ts),
            "14:43:49.038023456"
        );
        assert_eq!(
            StringFormat::UtcRFC2822.as_string(&ts),
            "Fri, 06 Mar 2026 14:43:49 +0000"
        );
        assert_eq!(
            StringFormat::UtcRFC3339.as_string(&ts),
            "2026-03-06T14:43:49Z"
        );
        assert_eq!(
            StringFormat::UtcHTTP.as_string(&ts),
