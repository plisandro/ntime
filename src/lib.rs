...
impl Timestamp {
    ...
    pub fn as_millis(&self) -> u128 {
        (self.seconds as u128) * U128_MILLIS_IN_SECOND
            + (self.nanoseconds as u128 / U128_NANOS_IN_MILLI)
    }
...
mod tests {
    ...
    fn comparison() {
        assert_eq!(
            Timestamp::new(1234, 100),
            Timestamp::new(1234, 100),
            "equal"
        );
        assert!(
            Timestamp::new(1234, 100) > Timestamp::new(1234, 50),
            "same secs, more millis"
        );
        assert!(
            Timestamp::new(1234, 100) < Timestamp::new(1234, 200),
            "same secs, less millis"
        );
        assert!(
            Timestamp::new(5678, 100) > Timestamp::new(1234, 100),
            "more secs"
        );
        assert!(
            Timestamp::new(1234, 100) < Timestamp::new(5768, 100),
            "less secs"
        );
    }
    ...
    fn operators() {
        assert_eq!(
            Timestamp::from_nanos(1234) - Timestamp::from_nanos(1234),
            Duration::ZERO,
            "zero result"
        );
        assert_eq!(
            Timestamp::from_nanos(1234) - Timestamp::from_nanos(5768),
            Duration::ZERO,
            "underflow"
        );
        assert_eq!(
            Timestamp::from_nanos(5678) - Timestamp::from_nanos(1234),
            Duration::from_nanos(4444),
            "OK"
        );
    }
